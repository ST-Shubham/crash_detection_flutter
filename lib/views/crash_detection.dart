...
class _GyroscopeDataCollectorState extends State<GyroscopeDataCollector> {
  ...
  late Timer timer;
  bool hasShownCrashDialog = false;
  bool hasShownHeartDialog = false;
  @override
  void initState() {
    _service = FirebaseCloudStorage();
    super.initState();
    predictionDataFuture = _service.getPredictions(documentId: user.id);
    healthDataFuture = _service.getHeathData(documentId: user.id);
    ...
    Timer.periodic(const Duration(milliseconds: 100000), (Timer timer) async {
      sensorData = {
        accx: _accelerometerData?.x ?? 0,
        accy: _accelerometerData?.y ?? 0,
        accz: _accelerometerData?.z ?? 0,
    ...
    timer = Timer.periodic(const Duration(seconds: 2), (Timer t) {
      if (shouldSyncData) {
        ...
        _service.getHeathData(documentId: user.id).then((data) {
          setState(() {
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasError) {
                  ...
                  if (predictionData.crashPrediction == 1 &&
                    ...
                    shouldSyncData = false;
                    // showCrashDialog(context);
                    hasShownHeartDialog = true;
                    // I want to call dialog here
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      showHeartAnomilityDialog(context);
                      // showDialog(
                      //   context: context,
                      //   builder: (BuildContext context) {
                      //     return AlertDialog(
                  ...
                  if (predictionData.heartRateAnomility == 0) {
                    hasShownHeartDialog = false;
                  }
                  if (predictionData.crashPrediction == 0) {
